...
#Url para los grupos
urlGrupos = 'http://www.ehu.eus/--/web/vicer.grado-innovacion/aurtengo-graduak-campus-ikastegia?p_p_id=upvehuapp_WAR_upvehuappportlet&p_p_state=normal&p_p_mode=view&p_p_col_id=column-2&p_p_col_pos=0&p_p_col_count=1&p_p_lifecycle=1&_upvehuapp_WAR_upvehuappportlet_action=redirectAction&reu=/pls/entrada/plew0040.htm_asignatura_next?p_sesion=&p_cod_idioma=CAS&p_en_portal=N&p_cod_centro=--&p_cod_plan=--&p_anyoAcad=act&p_pestanya=3&p_menu=principal&p_cod_asig=--&p_ciclo=X&p_curso=1&p_vengo_de=asig_cursos'

#Montar url de los grupos de la asignatura
def crearUrlGrupos(idioma, codCentro, codPlan, codAsig):
    grupoURL = urlGrupos.split('--')
    return grupoURL[0]+idioma+grupoURL[1]+codCentro+grupoURL[2]+codPlan+grupoURL[3]+codAsig+grupoURL[4]
...
def crearJsonGrupos(idioma, codCentro, codPlan, codAsig):
    grupos = []
    url = crearUrlGrupos(idioma, codCentro, codPlan, codAsig)
...
def obtenerGruposAsignaturaInformatica(codAsig):
    jsonGrupos = crearJsonGrupos('es', '226', 'GINFOR20', codAsig)
    return jsonGrupos
